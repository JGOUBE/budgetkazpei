    ...
    def classify(
        ...
        *,
        ...
        for line in lines:
            ...
            percentage_match = PERCENT_RE.search(text)
            percentage_discount = percentage_match is not None
            percentage_at_start = (
                percentage_match is not None
                and not re.search(
                    r"[A-Za-z0-9]",
                    text[:percentage_match.start()],
                )
            )
            digits = bool(re.search(r"\d", text))
            count = declared_count(text)
            printed_line_count = line_count_summary(text)
            multibuy = looks_like_multibuy_detail(text)
            weight = (
            ...
            if semantic_compact.startswith("TOTAL"):
            ...
            raw_leading = text.lstrip()
            if raw_leading.startswith((">", "»")):
                role_scores[LineRole.SECTION.value] += 1.0
                role_scores[LineRole.PRODUCT.value] -= 0.9
            ...
            if (
                any(label in normalized for label in CONTRIBUTION_LABELS)
                or "DONTDEEE" in contribution_compact
                or "DONTDEA" in contribution_compact
                or "PRIXHORS" in contribution_compact
                or "ECOPARTICIPATION" in contribution_compact
                or "ECOCONTRIBUTION" in contribution_compact
            ):
                role_scores[LineRole.TAX.value] += 1.0
                role_scores[LineRole.DETAIL.value] += 0.9
                role_scores[LineRole.PRODUCT.value] -= 1.0

            if semantic_compact in TABLE_TAX_LABELS:
                role_scores[LineRole.TAX.value] += 1.0
                role_scores[LineRole.PRODUCT.value] -= 0.9
            ...
            if (
                ...
                and not (
                    ...
                    or role_scores[LineRole.SECTION.value] >= 0.8
                    or role_scores[LineRole.TAX.value] >= 0.8
                    or role_scores[LineRole.TOTAL.value] >= 0.6
                )
            ):
            ...
            if (
                ...
                and role_scores[LineRole.SECTION.value] < 0.8
                and role_scores[LineRole.DISCOUNT.value] < 0.8
                and role_scores[LineRole.TAX.value] < 0.8
                and role_scores[LineRole.TOTAL.value] < 0.6
                and role_scores[LineRole.HEADER.value] < 0.8
                and role_scores[LineRole.FOOTER.value] < 0.8
            ):
    ...
    def _mark_group_subtotals(
        ...
        *,
        ...
        for index, line in enumerate(lines):
            if (
                is_summary_or_separator(line)
                or line.score_for(LineRole.SECTION) >= 0.8
            ):
    ...
    def _add_context_scores(lines: list[LineEvidence]) -> None:
        ...
        for index, line in enumerate(lines):
            ...
            if (
                ...
                and line.score_for(LineRole.SECTION) < 0.8
                and line.score_for(LineRole.DISCOUNT) < 0.8
                and line.score_for(LineRole.TAX) < 0.8
                and line.score_for(LineRole.TOTAL) < 0.6
                and line.score_for(LineRole.FOOTER) < 0.8
            ):
