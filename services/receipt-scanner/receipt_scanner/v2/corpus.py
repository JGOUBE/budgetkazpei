...
class CorpusResult:
    ...
    quality_budget_amount: float | None
    quality_should_record_budget: bool
    quality_article_total: float | None
    quality_payable_total: float | None
    quality_immediate_discount_total: float | None
    functional_total: float | None
    functional_total_source: str | None
    functional_total_match: bool | None
...
def _single_case(
    ...
    # Lazy import keeps manifest/report utilities usable in environments
    # where OCR model dependencies are intentionally not installed.
    ...
    return legacy, v2, quality_payload, elapsed
...
def _quality_amount(
    ...
    return parse_decimal(quality.get(key))


def _quality_receipt_amount(
    quality: dict[str, Any],
    key: str,
) -> Decimal | None:
    receipt = quality.get("receipt") or {}
    return parse_decimal(receipt.get(key))


def _functional_total(
    *,
    quality: dict[str, Any],
    v2_total: Decimal | None,
    legacy_total: Decimal | None,
) -> tuple[Decimal | None, str | None]:
    budget_amount = _quality_amount(quality, "budget_amount")
    if quality.get("should_record_budget") and budget_amount is not None:
        return budget_amount, "quality_budget_amount"
    if v2_total is not None:
        return v2_total, "v2_target_total"
    ...
    *,
    ...
    try:
        ...
        if case.mode == "long":
            ...
            legacy, v2, quality_payload, elapsed = _long_case(
                image_1,
                image_2,
                diagnostic_dir,
            )
        else:
            legacy, v2, quality_payload, elapsed = _single_case(
                image_1,
                diagnostic_dir,
            )
    ...
    quality_status = (quality_payload.get("status") or None)
    quality_budget_amount = _quality_amount(
        quality_payload,
        "budget_amount",
    )
    quality_article_total = _quality_receipt_amount(
        quality_payload,
        "article_total",
    )
    quality_payable_total = _quality_receipt_amount(
        quality_payload,
        "payable_total",
    )
    quality_immediate_discount_total = _quality_receipt_amount(
        quality_payload,
        "immediate_discount_total",
    )
    functional_total, functional_total_source = _functional_total(
    ...
    functional_total_match = _match_money(
        expected_total,
        functional_total,
    )
    ...
    return CorpusResult(
        ...
        quality_budget_amount=(
            float(quality_budget_amount)
            if quality_budget_amount is not None
            else None
        ),
        quality_should_record_budget=bool(
            quality_payload.get("should_record_budget")
        ),
        quality_article_total=(
            float(quality_article_total)
            if quality_article_total is not None
            else None
        ),
        quality_payable_total=(
            float(quality_payable_total)
            if quality_payable_total is not None
            else None
        ),
        quality_immediate_discount_total=(
            float(quality_immediate_discount_total)
            if quality_immediate_discount_total is not None
            else None
        ),
        functional_total=(
            float(functional_total)
            if functional_total is not None
            else None
        ),
        functional_total_source=functional_total_source,
        functional_total_match=functional_total_match,
        total_match=total_match,
...
def _write_html(
    ...
    for result in results:
        ...
        rows.append(
            ...
            "<td>{expected_total}</td><td>{functional_total}</td>"
            "<td>{functional_source}</td><td>{v2_total}</td>"
            "<td>{quality_budget}</td><td>{article_total}</td>"
            "<td>{payable_total}</td><td>{legacy_total}</td>"
            ...
            "</tr>".format(
                ...
                functional_total=_html_value(result.functional_total),
                functional_source=_html_value(result.functional_total_source),
                v2_total=_html_value(result.v2_total),
                quality_budget=_html_value(result.quality_budget_amount),
                article_total=_html_value(result.quality_article_total),
                payable_total=_html_value(result.quality_payable_total),
...
<th>Total attendu</th><th>Total fonctionnel</th><th>Source</th>
<th>Cible V2</th><th>Budget qualité</th><th>Total articles</th>
<th>Reste à payer</th><th>Total ancien</th>
...
def write_report(
    ...
    for result in results:
        lines.append(
            ...
            f"budget={result.functional_total} "
            f"({result.functional_total_source}) | "
            f"V2={result.v2_total} | "
            f"articles={result.quality_article_total} | "
            f"à_payer={result.quality_payable_total} | "
