...
class CorpusV2UtilitiesTest(unittest.TestCase):
    ...
    def test_functional_total_prefers_quality_budget_amount(self):
        total, source = _functional_total(
            quality={
                "should_record_budget": True,
                "budget_amount": 66.19,
            },
            v2_total=Decimal("66.34"),
            legacy_total=Decimal("66.34"),
        )
        self.assertEqual(total, Decimal("66.19"))
        self.assertEqual(source, "quality_budget_amount")

    def test_functional_total_falls_back_to_v2_when_quality_blocks(self):
        total, source = _functional_total(
            quality={
                "should_record_budget": False,
                "budget_amount": None,
            },
            v2_total=Decimal("4.59"),
            legacy_total=None,
        )
        self.assertEqual(total, Decimal("4.59"))
        self.assertEqual(source, "v2_target_total")
