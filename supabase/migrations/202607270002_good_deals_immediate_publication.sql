...
begin
  foreach required_table in array required_tables loop
    if to_regclass(required_table) is null then
      raise exception 'Migration 202607270002_good_deals_immediate_publication requires table % to exist before execution.', required_table;
    end if;
  end loop;

  if to_regprocedure('public.good_deals_is_admin()') is null then
    raise exception 'Migration 202607270002_good_deals_immediate_publication requires public.good_deals_is_admin() to exist before execution.';
  end if;

  if to_regprocedure('public.good_deals_normalize_text(text)') is null then
    raise exception 'Migration 202607270002_good_deals_immediate_publication requires public.good_deals_normalize_text(text) to exist before execution.';
  end if;
end $$;
...
begin
  ...
  if v_good_deal_id is null then
    ...
    set
      ...
      content_kind = v_candidate.content_kind,
      locality = v_candidate.locality,
