-- Alias manuels appris depuis les corrections d'articles.
-- Cette table reste anonymisee : aucune image, numero de ticket, carte de
-- fidelite, nom, email ou identifiant utilisateur n'y est stocke.
-- Rollback documente :
--   1. drop function if exists public.market_resolve_products_with_learned_aliases(jsonb);
--   2. drop function if exists public.market_learn_alias_from_receipt_item(uuid, text, boolean);
--   3. drop table if exists public.market_manual_product_aliases;

create extension if not exists pg_trgm;

create table if not exists public.market_manual_product_aliases (
  ...
  normalized_corrected_label text not null,
  scope text not null default 'chain',
  store_id uuid null references public.market_stores(id) on delete set null,
  store_chain_key text null,
  source text not null default 'user_manual_correction',
  status text not null default 'active',
  confidence numeric not null default 0.82,
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'market_manual_product_aliases_scope_valid'
      and conrelid = 'public.market_manual_product_aliases'::regclass
  ) then
    alter table public.market_manual_product_aliases
      add constraint market_manual_product_aliases_scope_valid
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'market_manual_product_aliases_status_valid'
      and conrelid = 'public.market_manual_product_aliases'::regclass
  ) then
    alter table public.market_manual_product_aliases
      add constraint market_manual_product_aliases_status_valid
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'market_manual_product_aliases_confidence_valid'
      and conrelid = 'public.market_manual_product_aliases'::regclass
  ) then
    alter table public.market_manual_product_aliases
      add constraint market_manual_product_aliases_confidence_valid
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'market_manual_product_aliases_validation_count_positive'
      and conrelid = 'public.market_manual_product_aliases'::regclass
  ) then
    alter table public.market_manual_product_aliases
      add constraint market_manual_product_aliases_validation_count_positive
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'market_manual_product_aliases_scope_coherent'
      and conrelid = 'public.market_manual_product_aliases'::regclass
  ) then
    alter table public.market_manual_product_aliases
      add constraint market_manual_product_aliases_scope_coherent
      check (
        (scope = 'store' and store_id is not null)
        or (scope = 'chain' and coalesce(length(trim(store_chain_key)), 0) > 0)
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'market_manual_product_aliases_labels_not_blank'
      and conrelid = 'public.market_manual_product_aliases'::regclass
  ) then
    alter table public.market_manual_product_aliases
      add constraint market_manual_product_aliases_labels_not_blank
      check (
        length(trim(raw_label)) > 0
        and length(trim(normalized_raw_label)) > 0
        and length(trim(corrected_label)) > 0
        and length(trim(normalized_corrected_label)) > 0
      );
  end if;
end $$;

create unique index if not exists market_manual_product_aliases_scope_uk
...
create index if not exists market_manual_product_aliases_exact_lookup_idx
  on public.market_manual_product_aliases (
    normalized_raw_label,
    status,
    scope,
...
create index if not exists market_manual_product_aliases_active_product_idx
  on public.market_manual_product_aliases (product_id, status, validation_count desc)
  where status = 'active';

create index if not exists market_manual_product_aliases_active_trgm_idx
  on public.market_manual_product_aliases
  using gin (normalized_raw_label gin_trgm_ops)
  where status = 'active';

comment on table public.market_manual_product_aliases is
  'Alias produits appris depuis des corrections manuelles, anonymises et scopes par magasin/enseigne/global.';
comment on column public.market_manual_product_aliases.raw_label is
  'Preuve OCR originale, jamais ecrasee.';
comment on column public.market_manual_product_aliases.corrected_label is
  'Correction utilisateur validee et persistante.';
comment on column public.market_manual_product_aliases.product_id is
  'Produit canonique associe dans market_products.';
comment on column public.market_manual_product_aliases.scope is
  'Portee de confiance: store, chain, global.';
comment on column public.market_manual_product_aliases.source is
  'Provenance de la correction, par exemple user_manual_correction.';
comment on column public.market_manual_product_aliases.status is
  'Etat de l alias: active, rejected, needs_review.';

alter table public.market_manual_product_aliases enable row level security;
revoke all on table public.market_manual_product_aliases from public, anon, authenticated;
grant all on table public.market_manual_product_aliases to postgres, service_role;
...
begin
  ...
  v_normalized_raw_label := public.market_normalize_text(v_raw_label);
  v_normalized_corrected_label := public.market_normalize_text(v_corrected_label);
  v_price := case
...
revoke execute on function public.market_resolve_products_with_learned_aliases(jsonb) from public;
revoke execute on function public.market_resolve_products_with_learned_aliases(jsonb) from anon;
revoke execute on function public.market_learn_alias_from_receipt_item(uuid, text, boolean) from public;
revoke execute on function public.market_learn_alias_from_receipt_item(uuid, text, boolean) from anon;
grant execute on function public.market_resolve_products_with_learned_aliases(jsonb) to postgres, service_role;
grant execute on function public.market_learn_alias_from_receipt_item(uuid, text, boolean) to authenticated, postgres, service_role;
