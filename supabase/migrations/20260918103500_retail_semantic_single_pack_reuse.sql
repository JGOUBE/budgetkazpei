-- BudgetKazPei - Retail auto-publication V1.6
--
-- A single product can be represented with package_count NULL or 1.
-- For exact semantic identity matching, those two values are equivalent.
-- All other identity fields remain strict.
  ...
  loop
    begin
      ...
      where p.normalized_name = v_candidate.normalized_product_name
        ...
        and coalesce(p.package_count, 1) = coalesce(v_candidate.pack_count, 1)
        and (
...
comment on function public.retail_prepare_safe_unmatched_candidates(text, uuid, integer) is
  'Safely reuses one exact semantic product for package-format wording mismatches; NULL and 1 package counts are equivalent for a single item.';
