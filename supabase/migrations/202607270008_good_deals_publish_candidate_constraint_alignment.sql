...
begin
  foreach required_table in array required_tables loop
    if to_regclass(required_table) is null then
      raise exception 'Migration 202607270008_good_deals_publish_candidate_constraint_alignment requires table % to exist before execution.', required_table;
    end if;
  end loop;

  if to_regprocedure('public.good_deals_is_admin()') is null then
    raise exception 'Migration 202607270008_good_deals_publish_candidate_constraint_alignment requires public.good_deals_is_admin() to exist before execution.';
  end if;

  if to_regprocedure('public.good_deals_normalize_text(text)') is null then
    raise exception 'Migration 202607270008_good_deals_publish_candidate_constraint_alignment requires public.good_deals_normalize_text(text) to exist before execution.';
  end if;
end $$;
...
declare
  ...
  v_catalog_source_kind public.shopping_catalogs.source_kind%type := 'collector';
  v_catalog_verification_status public.shopping_catalogs.verification_status%type := 'published';
  v_alias_source_kind public.shopping_product_aliases.source_kind%type := 'catalog';
  v_promotion_verification_status public.shopping_promotions.verification_status%type := 'published';
  v_candidate_published_status public.good_deal_candidates.status%type := 'published';
  ...
  v_good_deal_verification_status public.good_deals.verification_status%type := 'published'::public.good_deal_verification_status;
begin
  ...
  if nullif(btrim(coalesce(v_candidate.title, '')), '') is null then
    raise exception 'good deal title is required for immediate publication'
      using errcode = '22000';
  end if;

  if nullif(btrim(coalesce(v_candidate.description, '')), '') is null then
    raise exception 'good deal description is required for immediate publication'
      using errcode = '22000';
  end if;

  if v_candidate.starts_at is not null
     and v_candidate.ends_at is not null
     and v_candidate.ends_at < v_candidate.starts_at then
    raise exception 'invalid publication window: ends_at precedes starts_at for candidate %', p_candidate_id
      using errcode = '22000';
  end if;
  ...
  v_good_deal_availability_status := case
    when v_candidate.content_kind = 'event' then 'check_before_visit'
    when v_candidate.content_kind = 'permanent_leisure' then 'open'
    else 'unknown'
  end;

  if array_position(
    array['open', 'seasonal', 'temporarily_closed', 'check_before_visit', 'unknown'],
    v_good_deal_availability_status
  ) is null then
    raise exception 'invalid availability_status for good deal publication: %', coalesce(v_good_deal_availability_status, 'null')
      using errcode = '22000';
  end if;

  if array_position(
    array['official_catalog', 'official_offer', 'partner_feed', 'collector'],
    v_catalog_source_kind
  ) is null then
    raise exception 'invalid shopping catalog source_kind for immediate publication: %', coalesce(v_catalog_source_kind, 'null')
      using errcode = '22000';
  end if;

  if array_position(
    array['draft', 'published', 'expired', 'rejected'],
    v_catalog_verification_status
  ) is null then
    raise exception 'invalid shopping catalog verification_status for immediate publication: %', coalesce(v_catalog_verification_status, 'null')
      using errcode = '22000';
  end if;

  if array_position(
    array['receipt', 'catalog', 'manual', 'import'],
    v_alias_source_kind
  ) is null then
    raise exception 'invalid shopping product alias source_kind for immediate publication: %', coalesce(v_alias_source_kind, 'null')
      using errcode = '22000';
  end if;
  ...
  if array_position(
    ...
    raise exception 'invalid shopping promotion verification_status for immediate publication: %', coalesce(v_promotion_verification_status, 'null')
      using errcode = '22000';
  end if;

  if array_position(
    array['detected', 'needs_review', 'approved', 'published', 'rejected', 'duplicate', 'expired'],
    v_candidate_published_status
  ) is null then
    raise exception 'invalid good deal candidate status for immediate publication: %', coalesce(v_candidate_published_status, 'null')
      using errcode = '22000';
  end if;

  if v_good_deal_scope_type not in ('island', 'online')
     and nullif(
       btrim(
         coalesce(v_candidate.commune, v_candidate.micro_region, '')
       ),
       ''
     ) is null then
    raise exception 'scope_type % requires commune or micro_region for immediate publication', v_candidate.scope_type
      using errcode = '22000';
  end if;

  if v_product_promotion_candidate then
    if v_candidate.promo_price is not null and v_candidate.promo_price <= 0 then
      raise exception 'promo_price must be strictly positive for immediate publication'
        using errcode = '22000';
    end if;

    if v_candidate.original_price is not null and v_candidate.original_price <= 0 then
      raise exception 'original_price must be strictly positive for immediate publication'
        using errcode = '22000';
    end if;

    if v_candidate.unit_price is not null and v_candidate.unit_price <= 0 then
      raise exception 'unit_price must be strictly positive for immediate publication'
        using errcode = '22000';
    end if;

    if v_candidate.discount_percent is not null
       and (v_candidate.discount_percent < 0 or v_candidate.discount_percent > 100) then
      raise exception 'discount_percent must stay between 0 and 100 for immediate publication'
        using errcode = '22000';
    end if;

    if v_candidate.promo_price is not null
       and v_candidate.original_price is not null
       and v_candidate.original_price < v_candidate.promo_price then
      raise exception 'original_price must be greater than or equal to promo_price for immediate publication'
        using errcode = '22000';
    end if;
  end if;
  ...
  if v_product_promotion_candidate
     and v_candidate.commune is not null
     and nullif(btrim(coalesce(v_candidate.retailer_slug, '')), '') is not null then
    select id
  ...
  v_catalog_scope_type := case
    when v_candidate.scope_type = 'local' and v_store_location_id is not null then 'store'
    when v_candidate.scope_type = 'local' and nullif(btrim(coalesce(v_candidate.commune, '')), '') is not null then 'commune'
    when v_candidate.scope_type = 'nearby' and nullif(btrim(coalesce(v_candidate.micro_region, '')), '') is not null then 'micro_region'
    when v_candidate.scope_type = 'nearby' and nullif(btrim(coalesce(v_candidate.commune, '')), '') is not null then 'commune'
    when v_candidate.scope_type in ('commune', 'micro_region', 'island', 'online') then v_candidate.scope_type
    else null
  end;

  if (v_catalog_candidate or v_product_promotion_candidate)
     and v_catalog_scope_type is null then
    raise exception 'invalid shopping catalog scope_type for immediate publication: %', coalesce(v_candidate.scope_type, 'null')
      using errcode = '22000';
  end if;

  if (v_catalog_candidate or v_product_promotion_candidate)
     and array_position(
       array['island', 'micro_region', 'commune', 'store', 'online'],
       v_catalog_scope_type
     ) is null then
    raise exception 'unsupported shopping catalog scope_type for immediate publication: %', coalesce(v_catalog_scope_type, 'null')
      using errcode = '22000';
  end if;
  ...
  if v_catalog_candidate or v_product_promotion_candidate then
    ...
    if v_catalog_id is null then
      ...
      from public.shopping_catalogs
      where retailer_slug is not distinct from v_candidate.retailer_slug
        and title is not distinct from v_candidate.title
        and starts_at is not distinct from v_candidate.starts_at
        and ends_at is not distinct from v_candidate.ends_at
      limit 1;
    end if;
    ...
    if v_catalog_id is null then
      ...
      set
        ...
        source_kind = v_catalog_source_kind,
  ...
  set
    status = v_candidate_published_status,
