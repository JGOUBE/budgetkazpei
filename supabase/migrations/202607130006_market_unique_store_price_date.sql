-- Market V1.2 : résolution sûre par contexte exact unique.
-- Ajoute un chemin générique fondé sur magasin + date + prix exact + unicité
-- du produit, avec un plancher lexical pour éviter les associations arbitraires.
-- Aucun produit ni alias n'est codé en dur.
...
begin
  ...
  loop
    ...
    -- Cas générique à très forte preuve contextuelle : même magasin, même date,
    -- même prix au centime et un seul produit plausible lexicalement.
    -- Cette branche corrige les OCR déformés sans dépendre d'un alias exact et
    -- sans choisir arbitrairement lorsqu'au moins deux produits restent possibles.
    if v_match_type is null
      and v_normalized_name <> ''
      and v_observed_price is not null
      and v_observed_price > 0
      and v_observed_date is not null
    ...
    then
      ...
      exact_context_scores as (
        select
          products.id,
          coalesce(max(
            case when inputs.input_source = 'primary_vision' then
              greatest(
                similarity(products.normalized_name, inputs.candidate_name),
                coalesce(similarity(aliases.normalized_raw_label, inputs.candidate_name), 0)
        ...
        from public.market_price_observations observations
        join public.market_products products
          on products.id = observations.product_id
        cross join input_names inputs
        left join public.market_product_aliases aliases
          on aliases.product_id = products.id
        where observations.store_id = v_store_id
          and observations.observed_date = v_observed_date
          and round(observations.price, 2) = round(v_observed_price, 2)
        group by products.id
      ),
      eligible_exact_context as (
        select
          id,
        ...
        from exact_context_scores
        where greatest(primary_lexical_score, alternate_lexical_score) >= 0.25
      ),
      ranked_exact_context as (
        select
          eligible_exact_context.*,
          count(*) over () as candidate_count
        from eligible_exact_context
      )
      select *
      into v_candidate
      from ranked_exact_context
      order by lexical_score desc, id
      limit 1;

      if found and v_candidate.candidate_count = 1 then
        v_selected_product_id := v_candidate.id;
        v_match_input_source := v_candidate.match_input_source;
        v_match_type := case
          when v_candidate.match_input_source = 'alternate_ocr'
            then 'contextual_same_store_exact_price_date_alt_ocr'
          else 'contextual_same_store_exact_price_date'
        end;
        v_match_confidence := least(
          0.98,
          greatest(0.88, 0.88 + 0.10 * v_candidate.lexical_score)
        );
      end if;
    end if;
