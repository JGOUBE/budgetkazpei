...
export default function App() {
  ...
  return (
    <div style={{
      ...
      // ✅ Layout flex horizontal sur desktop
      ...
      {isMobile && (
        <div style={{
          ...
          <button onClick={() => setShowSidebar(!showSidebar)} style={{
            ...
            color: COLORS.text, fontSize: 22, cursor: "pointer", padding: 4,
          }}>
            {showSidebar ? "✕" : "☰"}
          </button>
      ...
      {isMobile ? (
        <div style={{
          position: "fixed", top: 0, left: 0, zIndex: 45, height: "100vh",
          transform: showSidebar ? "translateX(0)" : "translateX(-100%)",
          transition: "transform 0.3s ease",
        }}>
          <Sidebar activeNav={activeNav} onNavChange={handleNavChange}
            onSignOut={signOut} user={user} isPremium={isPremium} lang={lang} t={t} />
        </div>
      ) : (
        // ✅ Desktop : sidebar dans le flux flex, sticky
      ...
      <div style={{
        ...
        minWidth: 0, // ✅ Empêche le contenu de déborder
      }}>

        {/* Header desktop */}
        {!isMobile && (
          <Header activeNav={activeNav} onAdd={() => setShowModal(true)}
        ...
        {/* ── DASHBOARD ── */}
        {activeNav === "dashboard" && (
          <Dashboard stats={{ revenus, depenses, solde }}
            byCategory={byCategory} pieData={pieData}
            transactions={transactions} t={t} isMobile={isMobile} />
        )}
        ...
        {activeNav === "abonnements" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{
              ...
              <button style={{
                marginLeft: isMobile ? 0 : "auto", background: COLORS.green, border: "none",
                borderRadius: 10, padding: "10px 16px", color: "#fff",
                fontSize: 13, cursor: "pointer", fontFamily: "inherit", fontWeight: 600,
                width: isMobile ? "100%" : "auto",
              }}>{t("abonnements", "seeOffer")}</button>
            </div>
          </div>
        )}
