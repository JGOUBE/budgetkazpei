...
export async function runReceiptScannerFrontRegressionFixtures(): Promise<RegressionResult[]> {
  ...
  results.push(assertEqual("persist-corrected-needs-review-shopping-fed", correctedNeedsReviewMocks.calls.syncShoppingItemsFromReceipt.length, 1))
  results.push(assertEqual("persist-corrected-needs-review-shopping-count", correctedNeedsReviewMocks.calls.syncShoppingItemsFromReceipt[0]?.items?.length, 1))
