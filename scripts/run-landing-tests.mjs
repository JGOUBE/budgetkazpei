...
assert.match(content, /Retrouvez les promotions disponibles autour de votre commune\./, "promotion disclosure missing")
assert.match(content, /Les bons plans autour de vous\./, "local offers section missing")
assert.match(content, /Des idées de sorties en famille\./, "family section missing")
...
assert.match(publicHome, /href="\/mentions-legales"/, "legal notices must remain accessible from the public footer")
